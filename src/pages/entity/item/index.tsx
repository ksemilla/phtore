...
const EntityItem = () => {
  ...
  const {loading, data, error} = useQuery<{ item: ItemType }>(GET_ITEM, { variables: { id }, fetchPolicy: "no-cache" })
  const [getProduct] = useLazyQuery<{ product: ProductType }>(GET_PRODUCT, { variables: { id }, fetchPolicy: "no-cache" })
  ...
  useEffect(()=>{
    data && (() => {
      setItem(data.item)
      const obj = Object.fromEntries(data.item.instances[0].attributes.map(attr => [attr.name, attr.value]))
      setSelected(obj)
    })()
  }, [data])
  ...
  useEffect(()=>{
    ...
    data && (()=>{
      const prod = data.item.instances.find(instance => {
        return instance.attributes.map((attr) => attr.name in selected && selected[attr.name] === attr.value).every(val => val === true)
      })
      if (prod) {
